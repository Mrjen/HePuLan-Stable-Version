...
Page({
  ...
  onShareAppMessage: function (res) {
    ...
    let sence = wx.getStorageSync('sence');

    return {
      title: '快来跟我一起瓜分50元红包！',
      path: `/pages/redPack/redPack?unique_code=${that.data.unique_code}&scene=${sence}`,
      success: function (res) {
        // 转发成功
        console.log('转发红包成功：' + `unique_code=${that.data.unique_code}&scene=${sence}`)
        that.setData({
          redPackShow: false,
          inviteTip: true
        }, function () {
